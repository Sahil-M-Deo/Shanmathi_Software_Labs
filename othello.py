        ...
        for i in range(game.boardHEIGHT):
            for j in range(game.boardWIDTH):
                if game.board[i][j]==None:
                    score=0
                    directions=([game.board[i][j+1:],game.board[i][j-1::-1],game.board[(i+1):][j]],game.board[(i-1)::-1][j],(game.board[i-1::-1][j-1::-1]).diagonal(),(game.board[i-1::-1][j+1:]).diagonal(),(game.board[i+1:][j+1:]).diagonal(),(game.board[i+1:][j-1::-1]).diagonal())
